...
export const web3_transferAdmin = async (
  wallet: WalletContextState,
  newAdmin: string,
): Promise<string | null> => {
  if (wallet.publicKey === null) throw new WalletNotConnectedError();
  ...
  const tx = new Transaction().add(
    await program.methods
      .setAdmin(new PublicKey(newAdmin)) // new_authority
      .accounts({
        authority: wallet.publicKey,
        globalState: await keys.getGlobalStateKey(),
        systemProgram: SystemProgram.programId,
...
export const web3_setNewFeeWallet = async (
  wallet: WalletContextState,
  newFeeWallet: string,
): Promise<string | null> => {
  if (wallet.publicKey === null) throw new WalletNotConnectedError();
  ...
  const tx = new Transaction().add(
    await program.methods
      .setTreasury(new PublicKey(newFeeWallet)) // new_authority
      .accounts({
        authority: wallet.publicKey,
        globalState: await keys.getGlobalStateKey(),
        systemProgram: SystemProgram.programId,
